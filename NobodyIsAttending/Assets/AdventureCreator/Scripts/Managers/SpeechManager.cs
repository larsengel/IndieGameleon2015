	...
	public class SpeechManager : ScriptableObject
	{
		...
		public void ShowGUI ()
		{
			...
			if (scrollSubtitles || scrollNarration)
			{
				...
				textScrollCLip = (AudioClip) EditorGUILayout.ObjectField ("Default text scroll audio clip:", textScrollCLip, typeof (AudioClip), false);
			}
					...
					foreach (AC.Menu menu in menuManager.menus)
					{
						...
						foreach (MenuElement element in menu.elements)
						{
							...
							else if (element is MenuSavesList)
							{
								...
								if (button.saveListType == AC_SaveListType.Save && button.showNewSaveOption)
								{
			...
			if (File.Exists (fileName))
			{
				...
				for (int y = 1; y < csvOutput.GetLength (1); y++)
				{
					if (csvOutput [0,y] != null && csvOutput [0,y].Length > 0)
					{
						...
						{
							Debug.LogWarning ("Error importing translation (ID:" + csvOutput [0,y] + ") - make sure that the CSV file is delimited by a '" + CSVReader.csvDelimiter + "' character.");
						}
					}
				}
