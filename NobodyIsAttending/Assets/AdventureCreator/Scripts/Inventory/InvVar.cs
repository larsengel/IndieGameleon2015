		...
		public string GetDisplayValue (int languageNumber)
		{
			...
			else if (type == VariableType.PopUp)
			{
				if (val >= 0 && popUps.Length > val)
				{
					if (languageNumber > 0)
					{
						string popUpsString = SpeechManager.GetTranslation (GetPopUpsString (), popUpsLineID, languageNumber);
			...
			else if (type == VariableType.String)
			{
				if (languageNumber > 0)
				{
					return SpeechManager.GetTranslation (textVal, textValLineID, languageNumber);
				}
				return textVal;
			}
			return "";
