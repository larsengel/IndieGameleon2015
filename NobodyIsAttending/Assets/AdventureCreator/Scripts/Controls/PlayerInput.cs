	...
	public class PlayerInput : MonoBehaviour
	{
		...
		private void Awake ()
		{
			if (KickStarter.settingsManager)
			{
				...
				{
					Debug.Log ("Starting a non-First Person game with a locked cursor - is this correct?"); 
				}
			}
		...
		/**
		 * <summary>Replaces "Input.GetAxis", allowing for Ouya integration and input simulating.</summary>
		...
		/**
		 * <summary>Replaces "Input.GetButton", allowing for Ouya integration.</summary>
		...
		/**
		 * <summary>Replaces "Input.GetButton", allowing for Ouya integration and input simulating.</summary>
		 * <param name = "axis">The Input button to detect</param>
		...
		private void UpdateDrag ()
		{
			...
			if (mouseState == MouseState.HeldDown && dragState == DragState.None && KickStarter.stateHandler.CanInteract ())
			{
				Grab ();
			}
		...
		/**
		 * Unlocks the cursor so that it can be used to rotate a Moveable_PickUp object.
		 */
