		...
		public static void Play (Char speaker, string speakerName, int lineNumber, string language)
		{
			...
			if (lineNumber > -1 && speakerName != "" && KickStarter.speechManager.searchAudioFiles)
			{
				...
				if (lipSyncData != null)
				{
					...
					{
						Debug.LogWarning ("No LipSync component found on " + speaker.gameObject.name + " gameobject.");
					}
				}
				else
				{
					Debug.LogWarning ("No lipsync file found.  Looking for 'Resources/" + filename + "'");
				}
			}
			#else
			Debug.LogError ("The 'RogoLipSyncIsPresent' preprocessor define must be declared in the Player Settings.");
			#endif
		}
